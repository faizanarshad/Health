...
def test_list_doctors_seeded():
    ...
    assert "Dr. Emily Grant" in names
...
def test_check_availability_full_day():
    ...
    result = tools.check_availability("Dr. Emily Grant", future_date.strftime("%Y-%m-%d"))
    assert result["doctor"] == "Dr. Emily Grant"
...
def test_book_and_prevent_double_booking():
    ...
    booked = tools.book_appointment(
        "Jane Doe", "Dr. Emily Grant", date_str, "09:00", reason="checkup"
    )
    assert booked["status"] == "booked"

    clash = tools.book_appointment("John Roe", "Dr. Emily Grant", date_str, "09:00")
    assert "error" in clash


def test_book_rejects_invalid_time():
    with pytest.raises(ValueError):
        tools.book_appointment("Jane Doe", "Dr. Emily Grant", "2026-07-13", "09:15")
...
def test_cancel_appointment_frees_slot():
    ...
    while future_date.weekday() >= 5:
        future_date += timedelta(days=1)
    date_str = future_date.strftime("%Y-%m-%d")
    booked = tools.book_appointment("Jane Doe", "Dr. Emily Grant", date_str, "10:00")
    appointment_id = booked["appointment_id"]

    cancelled = tools.cancel_appointment(appointment_id)
    assert cancelled["status"] == "cancelled"

    availability = tools.check_availability("Dr. Emily Grant", date_str)
...
def test_list_appointments_only_shows_booked():
    ...
    while future_date.weekday() >= 5:
        future_date += timedelta(days=1)
    date_str = future_date.strftime("%Y-%m-%d")
    b1 = tools.book_appointment("Jane Doe", "Dr. Emily Grant", date_str, "09:00")
    tools.book_appointment("Jane Doe", "Dr. Michael Hart", date_str, "11:00")
    tools.cancel_appointment(b1["appointment_id"])

    appts = tools.list_appointments("Jane Doe")
    assert len(appts) == 1
    assert appts[0]["doctor_name"] == "Dr. Michael Hart"
...
def test_doctor_directory_has_profile_and_availability():
    ...
    emily = next(d for d in directory if d["name"] == "Dr. Emily Grant")
    assert emily["specialty"] == "Family Medicine"
    assert len(emily["availability_preview"]) == 6
...
def test_thread_lifecycle():
    ...
    ava = next(t for t in threads if t["patient_name"] == "Ava Miller")
    detail = tools.get_thread(ava["id"])
    assert detail is not None
    assert len(detail["messages"]) >= 2

    tools.add_thread_message(ava["id"], "patient", "Test message")
    updated = tools.get_thread(ava["id"])
